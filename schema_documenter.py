...
This script analyzes datasets and creates schema documentation files.
...
class SchemaDocumenter:
    def __init__(self, input_path: str):
        ...
        self.supported_extensions = {'.csv', '.xlsx', '.xls', '.json', '.dta', '.parquet', '.rds'}
        ...
        try:
            ...
            for column in df.columns:
                try:
                    ...
                    sample_values = [int(val) if isinstance(val, (np.integer, np.int64)) else val for val in sample_values]
    ...
    def save_schema(self, schema_info: Dict[str, Any], output_path: Path) -> None:
        ...
        """
        try:
            # Create output path with original extension + .schema
            original_extension = schema_info['file_type']
            schema_path = output_path.with_suffix(original_extension + '.schema')
            
            with open(schema_path, 'w') as f:
                json.dump(schema_info, f, indent=4, cls=NumpyEncoder)
            logger.info(f"Schema saved to {schema_path}")
        except Exception as e:
            logger.error(f"Error saving schema to {schema_path}: {str(e)}")
    ...
    def process_directory(self) -> None:
        ...
        if self.input_path.is_file():
            ...
            files = [f for f in self.input_path.rglob('*') if f.is_file() and self.is_supported_file(f)]
        ...
        for file_path in files:
            ...
            output_path = file_path.with_suffix('.schema')
            self.save_schema(schema_info, output_path)

def main():
    """
    Main function to run the schema documentation process.
    """
    import argparse
    
    parser = argparse.ArgumentParser(description='Document dataset schemas')
    parser.add_argument('input_path', help='Path to dataset file or directory')
    args = parser.parse_args()
    
    documenter = SchemaDocumenter(args.input_path)
